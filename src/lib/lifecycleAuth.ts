// Read-only auth for the Player Lifecycle analytics API. Same session-bearer pattern
// as crmAuth, but gates on can_access_lifecycle (the Player Lifecycle tab's
// permission) instead of chats. Admins pass. Returns a service-role client so the route can read the
...
export async function authenticateLifecycle(
  ...
  const appUser = await sb
    .from("app_users")
    .select("is_admin, can_access_lifecycle")
    .ilike("email", email)
    .maybeSingle();
  if (appUser.error || !appUser.data) {
    return { ok: false, status: 403, error: "Not a cockpit user" };
  }
  if (appUser.data.is_admin !== true && appUser.data.can_access_lifecycle !== true) {
    return { ok: false, status: 403, error: "Player Lifecycle access required" };
  }
