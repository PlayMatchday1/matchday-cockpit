...
// Shared icon rail for the two chat consoles (Match Chats + Player Chats),
// mockups matchops-chats-v1 / playerchats-v1. It REPLACES the shared
// SectionSideNav on the two /match-ops/*chats routes only (via an early return
// in the section layout); SectionSideNav itself is untouched and every other
// Match Ops page + all of Tech keep it. Two labelled groups (Operations,
// Conversations); the active item — a raised white pill with a 3px accent left
// edge — is driven by the route. Collapse (212 ⇄ 60px) is owned by each console
// and persisted there.
//
// Badges render only from real counts, hidden at zero (a false zero is worse
// than no badge): Player Chats shows the GLOBAL "waiting on a reply" count —
// the same number the Player Chats metrics-strip pill shows. Match Chats shows
// no badge (its waiting grouping is blocked; see the Match Chats ship report),
// and Review shows none (no truthful source).
...
const ICONS: Record<string, React.ReactNode> = {
  master: (<I><rect x="3" y="4" width="18" height="17" rx="2.5" /><path d="M3 9h18M8 2v4M16 2v4" /></I>),
  pipeline: (<I><path d="M3 5h18l-7 8v6l-4 2v-8z" /></I>),
  ops: (<I><rect x="2.5" y="5.5" width="19" height="13" rx="2" /><path d="M12 5.5v13" /><circle cx="12" cy="12" r="3" /></I>),
  review: (<I><path d="M9 11l2.5 2.5L16 8" /><rect x="3" y="3.5" width="18" height="17" rx="2.5" /></I>),
  match: (<I><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.9 8.9 0 0 1-3.8-.9L3 21l1.9-5.1A8.4 8.4 0 0 1 12 3.1a8.4 8.4 0 0 1 9 8.4z" /></I>),
  players: (<I><circle cx="9" cy="8" r="3.4" /><path d="M2.5 20a6.5 6.5 0 0 1 13 0" /><path d="M17 7.5a3 3 0 0 1 0 6M18.5 20a6 6 0 0 0-3-5.2" /></I>),
};
...
export default function ChatsRail({
  ...
  const items: Item[] = [];
  if (canCities) items.push({ group: "Operations", label: "Master Schedule", href: "/match-ops/master-schedule", icon: ICONS.master });
  if (canClub) items.push({ group: "Operations", label: "Field Pipeline", href: "/match-ops/field-pipeline", icon: ICONS.pipeline });
  if (canCities) items.push({ group: "Operations", label: "Field Ops", href: "/match-ops/field-ops", icon: ICONS.ops });
  if (isAdmin) items.push({ group: "Operations", label: "Review", href: "/match-ops/review", icon: ICONS.review });
  if (canChats) {
    items.push({ group: "Conversations", label: "Match Chats", href: "/match-ops/match-chats", icon: ICONS.match });
    items.push({ group: "Conversations", label: "Player Chats", href: "/match-ops/player-chats", icon: ICONS.players, count: awaiting });
  }
