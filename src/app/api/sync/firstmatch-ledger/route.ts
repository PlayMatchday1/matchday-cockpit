// POST /api/sync/firstmatch-ledger — manual trigger for the first-match
// abuse ledger sync. Backs the SyncCard "Sync now" button on /data →
// Finance / First-match ledger section.
//
...
//           firstmatch_ledger is a service-role-only table, so the actual
//           insert runs with the SERVICE ROLE, not the caller's RLS
//           client. The session is an AUTHZ gate only.
//   Cron:   Bearer ${CRON_SECRET} (constant-time). Service-role client.
// Either way the work runs through runWithLog("firstmatch-ledger") so the
// fin_sync_log row stays consistent with the cron path.
//
// On success: 200 with { ok: true, result: { inserted, ... } }.
...
// Scans the full mdapi_match_players table each run; insert-only. ~30s
// typical, 120s gives comfortable headroom.
...
export async function POST(req: Request) {
  ...
  // firstmatch_ledger is service-role-only, so the work uses the service
  // role in BOTH modes — the session is validated for authz only.
