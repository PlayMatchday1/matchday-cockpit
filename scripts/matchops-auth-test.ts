...
is("authenticateMatchOpsRead is imported by EXACTLY the 16 intended routes", importsMatchOpsRead.map(rel).sort(), [
  ...
  // Phase 31 — Player Finder, the parameterised list that REPLACED the Registered Players table on
  // the same page. Same gate for the same reason: a confined account must reach it, and this gate
  // is what hands the route its scope. It reads the 0133/0134 views (player_finder_rows,
  // player_finder_occupancy) rather than the base tables, but the auth surface is identical.
  "players/finder/route.ts",
