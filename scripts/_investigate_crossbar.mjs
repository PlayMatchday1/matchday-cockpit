...
// venue + field
const v=await sb.from("fin_venues").select("id, venue_name, city, billing_type, per_match_rate, cost_per_match, max_spots, charge_on_cancel").ilike("venue_name","%crossbar%");
console.log("fin_venues Crossbar:", JSON.stringify(v.data[0]));
...
console.log(`=> field_id=${FID}, charge_on_cancel=${v.data[0].charge_on_cancel}, max_spots=${defSpots}\n`);

// ===== PART 1: Jul 1-8 reconciliation =====
console.log("========== PART 1: Jul 1-8 reconciliation ==========");
...
console.log("\n--- GAPS: in mdapi but NOT in schedule_master ---");
const gaps=mmRows.filter(r=>!smKeys.has(`${r.date}|${r.hhmm}`));
if(!gaps.length)console.log("  (none)"); else gaps.forEach(r=>console.log(`  ${r.date} ${r.dow} ${r.time} (${r.cancelled?"cancelled":"alive"} in mdapi)`));

console.log("\n--- DRIFT: in schedule_master but NOT in mdapi (deleted_at NULL) ---");
const drift=sm1.data.filter(r=>!mmKeys.has(`${r.match_date}|${parseHHMM(r.match_time)}`));
if(!drift.length)console.log("  (none)"); else drift.forEach(r=>console.log(`  ${r.match_date} ${dw(r.match_date)} "${r.match_time}" — not in mdapi (leftover-plan OR upstream-deleted)`));

// ===== PART 2: future recurring =====
console.log("\n========== PART 2: future recurring Tue/Thu/Sun 8 PM ==========");
...
for (const d of expected){
  const dup=sm2Keys.has(`${d}|${parseHHMM("8:00 PM")}`);
  if(dup)conflicts++; else toAdd.push(d);
  console.log(`  ${d} ${dw(d)} 8:00 PM  ${dup?"** already exists **":"MISSING -> add"}`);
}
console.log("weekday spread:", JSON.stringify(expected.reduce((a,d)=>{a[dw(d)]=(a[dw(d)]||0)+1;return a;},{})),"(expect Thu4,Sun3,Tue3)");
console.log(`\nPart 2: ${toAdd.length} to add, ${conflicts} conflicts`);
console.log("toAdd:", JSON.stringify(toAdd));

const cnt=await sb.from("schedule_master").select("*",{count:"exact",head:true});
console.log("\nschedule_master current count:", cnt.count, "(expect 2139)");
